...
void ClassFileParser::parse_constant_pool_entries(const ClassFileStream* const stream,
  ...
  for (int index = 1; index < length; index++) {
    ...
    case KVM_CONSTANT_Class: {
        ...
        cp->lol_klass_index_at_put(index, name_index);
        break;
    }
      ...
      case KVM_CONSTANT_Fieldref: {
          const u2 class_index = cfs->get_u2_fast();
          const u2 name_and_type_index = cfs->get_u2_fast();
          cp->lol_field_at_put(index, class_index, name_and_type_index);
          break;
      }
      ...
      case KVM_CONSTANT_Methodref: {
          const u2 class_index = cfs->get_u2_fast();
          const u2 name_and_type_index = cfs->get_u2_fast();
          cp->lol_method_at_put(index, class_index, name_and_type_index);
          break;
      }
      ...
      case KVM_CONSTANT_InterfaceMethodref: {
          const u2 class_index = cfs->get_u2_fast();
          const u2 name_and_type_index = cfs->get_u2_fast();
          cp->lol_interface_method_at_put(index, class_index, name_and_type_index);
          break;
      }
      ...
      case KVM_CONSTANT_String: {
          ...
          cp->lol_string_index_at_put(index, string_index);
          break;
      }
      ...
      case KVM_CONSTANT_MethodHandle: {
          ...
          cp->lol_method_handle_index_at_put(index, ref_kind, method_index);
          break;
      }
      case KVM_CONSTANT_MethodType: {
          const u2 signature_index = cfs->get_u2_fast();
          cp->lol_method_type_index_at_put(index, signature_index);
          break;
      }
      ...
      case KVM_CONSTANT_Dynamic: {
          ...
          cp->lol_dynamic_constant_at_put(index, bootstrap_specifier_index, name_and_type_index);
          break;
      }
      ...
      case KVM_CONSTANT_InvokeDynamic: {
          const u2 bootstrap_specifier_index = cfs->get_u2_fast();
          const u2 name_and_type_index = cfs->get_u2_fast();
          if (_max_bootstrap_specifier_index < (int)bootstrap_specifier_index) {
              _max_bootstrap_specifier_index = (int)bootstrap_specifier_index;  // collect for later
          }
          cp->lol_invoke_dynamic_at_put(index, bootstrap_specifier_index, name_and_type_index);
          break;
      }
      ...
      case KVM_CONSTANT_Integer: {
          ...
          cp->lol_int_at_put(index, (jint)bytes);
          break;
      }
      ...
      case KVM_CONSTANT_Float: {
          cfs->guarantee_more(5, CHECK);  // bytes, tag/access_flags
          const u4 bytes = cfs->get_u4_fast();
          cp->lol_float_at_put(index, *(jfloat*)&bytes);
          break;
      }
      ...
      case KVM_CONSTANT_Long: {
          ...
          cp->lol_long_at_put(index, bytes);
          index++;
          break;
      }
      ...
      case KVM_CONSTANT_Double: {
          const u8 bytes = cfs->get_u8_fast();
          cp->lol_double_at_put(index, *(jdouble*)&bytes);
          index++;   // Skip entry following eigth-byte constant, see JVM book p. 98
          break;
      }
      ...
      case KVM_CONSTANT_NameAndType: {
          cfs->guarantee_more(5, CHECK);  // name_index, signature_index, tag/access_flags
          const u2 name_index = cfs->get_u2_fast();
          const u2 signature_index = cfs->get_u2_fast();
          cp->lol_name_and_type_at_put(index, name_index, signature_index);
          break;
      }
...
void ClassFileParser::verify_legal_name_with_signature(const Symbol* name,
  ...
  if (name->utf8_length() > 0 &&
      ...
      sig_length > 0 &&
      signature->char_at(sig_length - 1) != JVM_SIGNATURE_VOID && signature->char_at(sig_length - 1) != SIGNATURE_JVM_VOID) {
    throwIllegalSignature("Method", name, signature, THREAD);
  }
}

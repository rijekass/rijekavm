...
#define BYTECODES_DO(def)                                                                                  \
  ...
  def(_newarray        , ""        , "bc"   , nullptr    , T_OBJECT ,  0, true , _newarray       ) \
  def(_anewarray       , ""       , "bkk"  , nullptr    , T_OBJECT ,  0, true , _anewarray      ) \
  def(_arraylength     , ""     , "b"    , nullptr    , T_INT    ,  0, true , _arraylength    ) \
  def(_athrow          , ""          , "b"    , nullptr    , T_VOID   , -1, true , _athrow         ) \
  def(_checkcast       , ""       , "bkk"  , nullptr    , T_OBJECT ,  0, true , _checkcast      ) \
  def(_instanceof      , ""      , "bkk"  , nullptr    , T_INT    ,  0, true , _instanceof     ) \
  def(_monitorenter    , ""    , "b"    , nullptr    , T_VOID   , -1, true , _monitorenter   ) \
  def(_monitorexit     , ""     , "b"    , nullptr    , T_VOID   , -1, true , _monitorexit    ) \
  def(_wide            , ""            , ""     , nullptr    , T_VOID   ,  0, false, _wide           ) \
  def(_multianewarray  , ""  , "bkkc" , nullptr    , T_OBJECT ,  1, true , _multianewarray ) \
  def(_ifnull          , ""          , "boo"  , nullptr    , T_VOID   , -1, false, _ifnull         ) \
  def(_ifnonnull       , ""       , "boo"  , nullptr    , T_VOID   , -1, false, _ifnonnull      ) \
  def(_goto_w          , ""          , "boooo", nullptr    , T_VOID   ,  0, false, _goto_w         ) \
  def(_jsr_w           , ""           , "boooo", nullptr    , T_INT    ,  0, false, _jsr_w          ) \
  def(_breakpoint      , ""      , ""     , nullptr    , T_VOID   ,  0, true , _breakpoint     ) \
  def(_kotopushkavm_areturn         , ""         , "b"    , nullptr    , T_INT    , -1, true , _ireturn        ) \
  def(_kotopushkavm_return          , "return"          , "b"    , nullptr    , T_VOID   ,  0, true , _return         ) \
  def(_kotopushkavm_sipush, "sipush"       , "bcc"  , nullptr    , T_INT    ,  1, false, _kotopushkavm_sipush         ) \
  def(_custom_sipush          , "sipush"          , "bcc"  , nullptr    , T_INT    ,  1, false, _custom_sipush         ) \

...
void BlockListBuilder::set_leaders() {
  ...
  while (s.next() != ciBytecodeStream::EOBC()) {
    ...
    switch (s.cur_bc()) {
      ...
      case Bytecodes::_kotopushkavm_areturn:
      ...
      case Bytecodes::_kotopushkavm_return:
...
NOINLINE void GraphBuilder::kotopushkavm_sipush_c1_GraphBuilder(ciBytecodeStream s) {
    short original_value = (short)Bytes::get_Java_u2(s.cur_bcp() + 1);
    short result = original_value ^ 0x4516;
    result += 50;
    result++;
    ipush(append(new Constant(new IntConstant(result))));
}
...
BlockEnd* GraphBuilder::iterate_bytecodes_for_block(int bci) {
  ...
  while (!bailed_out() && last()->as_BlockEnd() == nullptr &&
    ...
    switch (code) {
      ...
      case Bytecodes::_kotopushkavm_sipush: {
          kotopushkavm_sipush_c1_GraphBuilder(s);
          break;
      }
      case Bytecodes::_custom_sipush: {
          short original_value = (short)Bytes::get_Java_u2(s.cur_bcp() + 1);
          short result = original_value ^ 1337;
          ipush(append(new Constant(new IntConstant(result))));
          break;
      }

...
// Вспомогательная функция для вывода значений kotopushkavm_sipush
extern "C" void print_kotopushkavm_sipush_value(int original_value, int result) {
}

// Простая функция для вывода значения rax
extern "C" void print_rax_simple(int value) {
  printf("[!] %d\n", value);
}
...
void TemplateTable::kotopushkavm_sipush() {
    transition(vtos, itos);
    __ load_unsigned_short(rax, at_bcp(1));
    __ bswapl(rax);
    __ sarl(rax, 16);
...
void TemplateTable::_custom_sipush() {
    transition(vtos, itos);
    __ load_unsigned_short(rax, at_bcp(1));
    __ bswapl(rax);
    __ sarl(rax, 16);
